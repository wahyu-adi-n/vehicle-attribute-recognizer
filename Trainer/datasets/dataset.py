...
class CarsDataModule(DataModuleBase):
    ...
    def prepare_dataset(self, cfg):
        self.train_set = ImageFolder(root=cfg['dataset']['train_dir'],
                                     transform=self.train_transforms)
        self.val_set = ImageFolder(root_dir=cfg['dataset']['val_dir'],
                                   transform=self.test_val_transforms)
        self.test_set = ImageFolder(root_dir=cfg['dataset']['test_dir'],
                                    transform=self.test_val_transforms)

    def train_dataloader(self, cfg):
        kwargs = dict(
