...
if __name__ == '__main__':
    ...
    parser.add_argument('-td', '--testdir', 
                        default='../Dataset/car_data/car_data/test/*/*.jpg', 
...
def main(args):
    ...
    for image_path in test_images:
        ...
        print(
            f"Actual: {actual_class_name} / Prediction: {pred_class_name} ({conf_score})")
        print(f"Inference Time: {(end_time - start_time)*1000 :.3f} ms.")
    ...
    if args.savemetrics:
        classification_report = metrics_report_to_df(
            y_true, y_pred, args.savedir)
        save_plot_cm(y_true, y_pred, args.savedir)
        print(
            f"Precision score (weighted): {prec_score(y_true, y_pred, class_names):.3f}")
        print(
            f"Recall score (weighted): {recc_score(y_true, y_pred, class_names):.3f}")
        print(f"Accuracy score (weighted): {acc_score(y_true, y_pred):.3f}")
        print(
            f"F1 Score score (weighted): {fone_score(y_true, y_pred, class_names):.3f}")
        print(f"Clssification report:\n")
        print(classification_reports(y_true, y_pred, class_names))
